...
**Frente 1: B + C juntos** — ✅ **CERRADO 6-ago-2026, desplegado en `d1f53ee`.**
«Detener el daño». Eran los dos únicos frentes mecánicos: no pedían criterio de diseño,
se verificaban objetivamente y estaban costando catálogo y exposición legal cada día.
Spec en `2026-08-06-frente-b-c-rutas-y-atribucion-design.md`, plan en
`../plans/2026-08-06-frente-b-c-rutas-y-atribucion.md`.

Verificado en producción tras el pull del cron:

| Pieza | Resultado |
|---|---|
| B1 sitemap | `built`/`destacados` fuera; `glosario` dentro (control del fail-open: el Hub respondía, así que el 0 es real); 182 URLs de 184 |
| B2 facetas | 6·5·5 (etapa) y 6·3·2·1·0 (tipo) — los ocho conteos exactos |
| B3 listado móvil | 0 scrollers anidados, documento de 14.815 px, tarjetas de 374 px, pie tras la última; escritorio conserva su split |
| C1 atribución | Cero nombres de proveedor en zonas, `/es/rentas` y la pestaña tradicional de `/es/mercado`; ambas muestran «Análisis de mercado Propyte» |
| Regresión | Las 92 rutas ES responden, ninguna 4xx/5xx |

Tres números del informe original corregidos al medir: `/tipo/terreno` da **1**, no 2 (un
mismo desarrollo tiene `Terrenos` y `Lotes`, y `overlaps` lo cuenta una vez); el sitemap
queda en **182**, no 180 (`/en/built` no estaba); y **B3 era P1, no P0** — la lista sí era
alcanzable, por un scroller anidado.

Pendiente de decisión de negocio, sin código: **`/desarrollos/tipo/macrolote` seguirá
vacía** porque ese valor no existe en el inventario. O se retira la faceta y su entrada del
sitemap, o se acepta declarada como vacía.

**Frente 2 recomendado: A — verdad del dato.** Sostiene la única ventaja diferenciada del
producto y desbloquea G. Luego **D** (mayor alcance por páginas, con la victoria más barata
del informe: un token de color arregla 45), después **F**, **G** y **E**. Sin spec todavía.
