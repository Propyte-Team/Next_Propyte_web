        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-16">
          {audience.map((aud) => {
            ...
            return (
              ...
              >
                <div className="w-12 h-12 mb-4 bg-propyte-brand/20 rounded-lg flex items-center justify-center">
                  <Icon size={24} strokeWidth={1.75} className="text-teal-a11y" />
                </div>
                <h3 className="text-lg font-semibold text-graphite mb-2">{t(aud.titleKey)}</h3>
                <p className="text-gray-600 mb-4">{t(aud.descKey)}</p>
                <Link
                  href={aud.href}
                  className="inline-flex items-center gap-1.5 min-h-[44px] md:min-h-0 text-teal-a11y font-medium hover:underline"
                >
                  {t(aud.ctaKey)}
                  <ArrowRight size={14} />
                </Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((feat, i) => {
            ...
            return (
              ...
              >
                <div className="w-12 h-12 bg-propyte-brand/20 rounded-xl flex items-center justify-center mb-4">
                  <Icon size={24} strokeWidth={1.75} className="text-teal-a11y" />
                </div>
                <h3 className="text-lg font-bold text-graphite mb-2">{feat.title}</h3>
                <p className="text-sm text-gray-600 leading-relaxed mb-4 flex-1">{feat.desc}</p>
                <Link
                  href={feat.href}
                  className="inline-flex items-center gap-1.5 min-h-[44px] md:min-h-0 text-teal-a11y font-medium text-sm hover:underline mt-auto"
                >
                  {feat.cta}
                  <ArrowRight size={14} />
                </Link>
