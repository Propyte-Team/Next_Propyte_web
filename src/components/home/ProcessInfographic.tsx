...
// ─── Icon catalogue ──────────────────────────────────────────────────────
// Iconos canónicos Propyte (`@/lib/icons`). Heredan color vía currentColor del
// contenedor y llenan su span con `w-full h-full`. El color/stroke se controla
// desde los wrappers que ya existen abajo.

const ICON_CLS = 'w-full h-full';

const IconStart = <Laptop className={ICON_CLS} />;
const IconCheckMini = <Check className={ICON_CLS} strokeWidth={3} />;

// Panel A — SIN PROPYTE (confusión, saturación, arrepentimiento)
const ICONS_A: React.ReactNode[] = [
  <HelpCircle key="a1" className={ICON_CLS} />,
  <Phone key="a2" className={ICON_CLS} />,
  <Home key="a3" className={ICON_CLS} />,
  <Pencil key="a4" className={ICON_CLS} />,
  <Frown key="a5" className={ICON_CLS} />,
  <CircleX key="a6" className={ICON_CLS} />,
];

// Panel B — CON PROPYTE (perfil, mercado, visitas, negociación, entrega, partners, venta)
const ICONS_B: React.ReactNode[] = [
  <Users key="b1" className={ICON_CLS} />,
  <ChartColumnDecreasing key="b2" className={ICON_CLS} />,
  <Calendar key="b3" className={ICON_CLS} />,
  <ArrowDownUp key="b4" className={ICON_CLS} />,
  <CheckCircle key="b5" className={ICON_CLS} />,
  <House key="b6" className={ICON_CLS} />,
  <TrendingUp key="b7" className={ICON_CLS} />,
];

const PILLAR_ICONS: React.ReactNode[] = [
  <Clock key="p1" className={ICON_CLS} />,
  <Shield key="p2" className={ICON_CLS} />,
  <CreditCard key="p3" className={ICON_CLS} />,
  <ChartColumnDecreasing key="p4" className={ICON_CLS} />,
];

const IconAlertCircle = <AlertCircle className={ICON_CLS} />;
const IconCheck = <Check className={ICON_CLS} />;
const IconGhost = <Ghost className={ICON_CLS} />;
const IconChevron = <ChevronDown className={ICON_CLS} strokeWidth={2.5} />;
