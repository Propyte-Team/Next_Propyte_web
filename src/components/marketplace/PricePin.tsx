...
export default function PricePin({ price, isActive = false }: PricePinProps) {
  return (
    <div className={`relative inline-block cursor-pointer transition-transform ${isActive ? 'scale-110 z-10' : ''}`}>
      <div className={`px-2 py-1 rounded text-xs font-bold whitespace-nowrap shadow-[0_2px_6px_rgba(0,0,0,0.2)] ${isActive ? 'bg-[#5CE0D2] text-[#0F1923]' : 'bg-[#1A2F3F] text-white'}`}>
        {price}
      </div>
