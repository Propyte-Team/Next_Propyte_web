...
interface InversionInicialCalculatorProps {
  ...
  onMobiliarioNivel: (n: NivelAcabado) => void;
  decoracionNivel: NivelAcabado;
  onDecoracionNivel: (n: NivelAcabado) => void;
...
export default function InversionInicialCalculator({
  price, priceOriginal, discountPct, nacionalidad, onNacionalidad,
  mobiliarioNivel, onMobiliarioNivel, decoracionNivel, onDecoracionNivel, inversion,
    ...
    <div className="rounded-2xl border border-gray-200 p-5 space-y-5">
      ...
      <div>
        ...
        <div className="inline-flex rounded-lg border border-gray-200 overflow-hidden">
          <button type="button" onClick={() => onNacionalidad('nacional')}
          ...
          <button type="button" onClick={() => onNacionalidad('extranjero')}
        ...
        <div className="mt-2 text-sm text-gray-700">
          {formatPrice(inversion.escrituracion)}
          {nacionalidad === 'extranjero' && <span className="text-2xs text-gray-500 ml-2">{t('fideicomisoNota')}</span>}
        </div>
      </div>

      <AcabadoRow label={t('mobiliario')} nivel={mobiliarioNivel} setNivel={onMobiliarioNivel} incluido={inversion.mobiliarioIncluido} monto={inversion.mobiliario} t={t} />
      <AcabadoRow label={t('decoracion')} nivel={decoracionNivel} setNivel={onDecoracionNivel} incluido={inversion.decoracionIncluido} monto={inversion.decoracion} t={t} />

      <div className="bg-[#0F1923] rounded-2xl p-5 text-white">
        <div className="flex justify-between text-sm border-b border-white/10 pb-2"><span className="text-gray-400">{t('enganche')}</span><span className="font-semibold">{formatPrice(inversion.enganche)}</span></div>
        <div className="flex justify-between text-sm pt-2"><span className="text-gray-400">{t('escrituracion')}</span><span>{formatPrice(inversion.escrituracion)}</span></div>
        <div className="flex justify-between text-sm pt-1"><span className="text-gray-400">{t('mobiliario')}</span><span>{inversion.mobiliarioIncluido ? t('incluido') : formatPrice(inversion.mobiliario)}</span></div>
        <div className="flex justify-between text-sm pt-1"><span className="text-gray-400">{t('decoracion')}</span><span>{inversion.decoracionIncluido ? t('incluido') : formatPrice(inversion.decoracion)}</span></div>
        <div className="flex justify-between pt-3 mt-2 border-t border-white/10"><span className="font-bold text-propyte-brand">{t('inversionInicial')}</span><span className="text-xl font-extrabold text-propyte-brand">{formatPrice(inversion.total)}</span></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3">
        <select disabled={incluido} value={nivel} onChange={(e) => setNivel(e.target.value as NivelAcabado)}
          className={`${selectClass} max-w-[160px] ${incluido ? 'opacity-40 cursor-not-allowed' : ''}`}>
          <option value="standard">{t('standard')}</option>
          <option value="alto">{t('alto')}</option>
          <option value="premium">{t('premium')}</option>
        </select>
        <span className="text-sm font-semibold text-gray-800">{incluido ? <span className="text-emerald-600">{t('incluido')}</span> : formatPrice(monto)}</span>
      </div>
    </div>
  );
}
