...
export default async function MarketIndicator({
  ...
  const grade: { label: string; color: string; bg: string } =
    total >= 80
      ...
      : total >= 60
        ? { label: t('strong'), color: 'text-[#0E7490]', bg: 'bg-propyte-cyan-100 border-propyte-brand/30' }
        : total >= 40
  ...
  return (
    <div>
      ...
      <div className="h-1.5 bg-white/60 rounded-full overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-propyte-brand to-[#0E7490] rounded-full"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
