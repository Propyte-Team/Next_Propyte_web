...
export default async function MarketplacePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    ...
    if (supabase) {
      ...
      if (data) {
        ...
        const resolved = await resolveInvestmentForRows(supabase, rawUnits);
        properties = rawUnits.map((u) => mapUnitToProperty(u, locale, resolved.get(u.id)));
      }
    }
  } catch (error) {
  ...
  return (
    <>
      ...
      />
      <MarketplaceContent properties={properties} showMap heroHidden />
    </>
  );
}
